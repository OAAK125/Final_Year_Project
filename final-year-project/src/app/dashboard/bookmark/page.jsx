...
const BookmarkPage = () => {
  ...
  useEffect(() => {
    const fetchBookmarkedCertifications = async () => {
      ...
      const bookmarkedOnly = (data || []).filter((cert) =>
        bookmarkedIds.has(cert.id)
      );

      const transformed = bookmarkedOnly.map((cert) => {
    ...
    <section className="p-5 space-y-6 space-x-10">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Browse through your favourited practice tests.
        </p>
      </div>
      ...
      <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
        ...
        <div className="flex justify-between items-center gap-4">
          ...
          <button
            ...
            onClick={() => {
              setFilters({ certificationType: "", topic: "", arrangement: "" });
